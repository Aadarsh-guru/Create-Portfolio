...
const formSchema = z.object({
    customLogo: z.string().url({ message: "Invalid URL." }),
});

const LogoForm = ({ logo, userId }: LogoFormProps) => {
    ...
    const onDelete = async () => {
        setIsDeleting(true);
        try {
            const { success } = await customizeAction(userId, { customLogo: null });
            if (success) {
                const res = await deleteMediaAction(logo!);
                if (res.success) {
                    router.refresh();
                    toast({
    ...
    const hanldeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 1000000) {
            setError("Logo should be less than 1 MB.");
            return;
        }
        setIsUploading(true);
        try {
            const mediaUrl = await uploadMedia(file, `logos`);
            if (mediaUrl) {
                onSubmit({ customLogo: mediaUrl });
            };
        } catch (error: any) {
            console.log(error);
            return toast({
                ...
                {!isUploading && (
                    <>
                        ...
                        <div className="absolute inset-0 rounded-lg transition-all bg-black/75 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100">
                            ...
                            <p className="text-slate-50 text-sm font-semibold mt-2" >Upload new logo</p>
                        </div>
                    </>
                )}
            </div>
            ...
            <div className="flex items-center justify-between mt-4 gap-x-2">
                ...
                <ConfirmModel onConfirm={onDelete} >
                    <Button
                        ...
                        disabled={isDeleting || !logo || isUploading}
                    >
                        Remove
                    </Button>
                </ConfirmModel>
