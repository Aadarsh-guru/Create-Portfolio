...
import { createSocialLinkAction, editSocialLinkAction } from "@/actions/social";
...
interface EditSocialLinkModelProps {
    socialId: string;
    url: string;
...
function EditSocialLinkModel({ socialId, url, provider }: EditSocialLinkModelProps) {
    ...
    const onSubmit = async ({ url, provider }: z.infer<typeof formSchema>) => {
        try {
            const { success, message } = await editSocialLinkAction({ socialId, url, provider });
            if (success) {
                router.refresh();
                setOpen(false);
                toast({
...
export default EditSocialLinkModel;